...
/* rough correction from universal time to dynamical time,
   with both expressed as julian dates.  Based on
   1960 -- 1990 values, which look to form a pretty
   straight line.  Note that accurate values of the correction
   can only be derived from observations after the fact, and
   that the long-term behavior is not expected to be linear! */
...

{
	...
	x = 33.15 + (2.164e-3) * (jd - 2436934.5);
/*	printf("Correction %f sec \n",x);  */
	return(jd + x / SEC_IN_DAY);
}
...
short option;   /* 1 = 1 month per page, 2 = 2 months per page */

{
  ...
  printf("%% The rest of this should be essentially system-independent:\n");
  printf("\\nopagenumbers\n");
...

{
	...
	printf("  At the given twilight, the center of the sun is 18.0 degrees");
...
/*	Introductory banner for user, with site information and
	...
	printf("2 for TeX-style output with two months per page:");
	scanf("%hd",&TeX_out);
	if(TeX_out != 0)
              printf("\nYou'll have to edit the TeX output ... look for 'CUT HERE'\n");
	printf("\nThis program takes a while to run and \n");
	printf("produces wide and voluminous output.  You probably want to\n");
	printf("run it in background.  See source code or documentation\n");
...
/* there follows some awful flow of control.  This is an artifact of the
   ...
   to a lot of really weird branching.  Sorry.  It appears to work, though. */
		...
		if(jdsunset > 0.) /* convergence */
	      ...
	      TWILIGHT: hatwilight = ha_alt(decsun,lat,-18.);
		...
		jdetwilight = jd_sun_alt(-18.,jdetwilight,lat,longit);
		printf(" ");
		if(jdetwilight > 0.)
		...
		jdmtwilight = jd_sun_alt(-18.,jdmtwilight,lat,longit);
		printf(" ");
		if(jdmtwilight > 0.)
